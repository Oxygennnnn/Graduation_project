"""ConvNeXt V2 training script (train/valid/test).

This script is designed to be directly comparable with `vision_transformer.py`:
  - Same ImageFolder layout: <data_root>/{train,valid,test}/<class_name>/*
  - Same preprocessing/augmentation + normalization
  - Same LR range-test (auto LR) + early stopping logic
  - Same concise epoch logging
  - Saves best checkpoint each time val loss improves (overwrites one file)

Model:
  - Prefer torchvision ConvNeXtV2 if available.
  - Otherwise falls back to timm ConvNeXtV2 (requires `pip install timm`).

Example:
  python convnext_v2.py --batch_size 32 --epochs 30 --patience 5 --seed 42
"""
...
def build_transforms(img_size: int) -> tuple[transforms.Compose, transforms.Compose]:
	"""Keep identical preprocessing to vision_transformer.py for fair comparison."""
	aug_list = []
...
def lr_range_test(
	...
	"""Lightweight LR finder (same logic as vision_transformer.py)."""
	if num_iter <= 10:
...
def create_model(num_classes: int, model_name: str) -> nn.Module:
	...
	Order:
	  1) torchvision ConvNeXtV2 if available in the installed torchvision
	  2) timm convnextv2* fallback
	"""
	try:
		from torchvision import models
		# Try common torchvision naming for ConvNeXtV2
		if hasattr(models, "convnextv2_tiny") and model_name == "convnextv2_tiny":
			weights = None
			# weights enum may or may not exist
			if hasattr(models, "ConvNeXt_V2_Tiny_Weights"):
				weights = models.ConvNeXt_V2_Tiny_Weights.IMAGENET1K_V1
				m = models.convnextv2_tiny(weights=weights)
			else:
				m = models.convnextv2_tiny(weights=None)

			# Replace classifier head
			in_features = m.classifier[-1].in_features
			m.classifier[-1] = nn.Linear(in_features, num_classes)
			return m
		# Add more torchvision variants if present
		if hasattr(models, "convnextv2_base") and model_name == "convnextv2_base":
			weights = None
			if hasattr(models, "ConvNeXt_V2_Base_Weights"):
				weights = models.ConvNeXt_V2_Base_Weights.IMAGENET1K_V1
				m = models.convnextv2_base(weights=weights)
			else:
				m = models.convnextv2_base(weights=None)
			in_features = m.classifier[-1].in_features
			m.classifier[-1] = nn.Linear(in_features, num_classes)
			return m
	except Exception:
	...
	# timm fallback (loaded dynamically to avoid hard dependency for editor/linters)
	try:
		timm = importlib.import_module("timm")
	except Exception as e:
		raise RuntimeError(
			"ConvNeXt V2 model was not found in torchvision, and timm is not available. "
			"Install timm with: pip install timm"
		) from e

	# Try a few common timm names
	candidates = [
		model_name,
		# common pretrained identifiers
		f"{model_name}.fcmae_ft_in1k",
		f"{model_name}.fcmae",
		f"{model_name}.in1k",
	]
	...
	raise RuntimeError(
		f"Failed to create ConvNeXtV2 model from timm. Tried: {candidates}. "
		f"Last error: {last_err}"
	)


def main(argv: list[str] | None = None) -> int:
	parser = argparse.ArgumentParser(description="Fine-tune ConvNeXt V2 on ImageFolder dataset")
	...
	parser.add_argument("--output", type=Path, default=Path("convnextv2_best.pt"), help="Path to save best model")
	parser.add_argument(
		"--model",
		type=str,
		default="convnextv2_tiny",
		help="ConvNeXtV2 variant (tries torchvision first, else timm), e.g. convnextv2_tiny / convnextv2_base",
	)
	...
	# CUDA performance knobs (same as vision_transformer.py)
	...
	model = create_model(num_classes=num_classes, model_name=args.model).to(device)
	...
	print(
		f"Device=cuda | model={args.model} | classes={num_classes} | train={len(train_ds)} valid={len(valid_ds)} test={len(test_ds)} | lr={lr:.2e} | amp={'on' if use_amp else 'off'}"
	)
